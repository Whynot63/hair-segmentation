...
def _conv_block(inputs, filters, kernel=(3, 3), strides=(1, 1)):
    ...
    x = layers.ZeroPadding2D(
        padding=(1, 1), data_format=image_data_format, name="conv1_pad"
    )(inputs)
    x = layers.Conv2D(
        filters,
        kernel,
    ...
    x = layers.BatchNormalization(
        axis=channel_axis, momentum=0.1, epsilon=1e-05, name="conv1_bn"
    )(x)
...
def YellowBlock(inputs):
    return layers.UpSampling2D(size=(2, 2), data_format=image_data_format)(
        inputs
    )
...
def OrangeBlock(inputs, filters, block_id, kernel_size=(3, 3), stride=(1, 1)):
    x = layers.ZeroPadding2D(
        padding=(1, 1),
        data_format=image_data_format,
        name="conv_pad_orange_%d" % block_id,
    )(inputs)
    x = layers.SeparableConv2D(
        ...
        use_bias=False,
        data_format=image_data_format,
        name="sep_conv_orange_%d" % block_id,
    )(x)
